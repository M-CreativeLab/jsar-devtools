...
import { changeRoomDimension } from './core/messenger';

export function Inspector({ device, inputMode }) {
	const sceneContainerRef = React.useRef<HTMLDivElement>();
	// plane setting refs
	const planeWidthRef = React.useRef<HTMLInputElement>();
	const planeHeightRef = React.useRef<HTMLInputElement>();
	const [planeVertical, setPlaneVertical] = React.useState(true);
	const planeSemanticLabelRef = React.useRef<HTMLSelectElement>();
	// mesh setting refs
	const meshWidthRef = React.useRef<HTMLInputElement>();
	const meshHeightRef = React.useRef<HTMLInputElement>();
	const meshDepthRef = React.useRef<HTMLInputElement>();
	const meshSemanticLabelRef = React.useRef<HTMLSelectElement>();
				...
				{showRoomSettings && (
					<div className="row transform-card" id="room-dimension-settings">
						<div className="col-2 transform-icon d-flex justify-content-center align-items-center">
							<img
								src="./res/icons/xr-emulator/roomscale.png"
								className="control-icon"
							/>
						</div>
						<div className="col-10 transform-body">

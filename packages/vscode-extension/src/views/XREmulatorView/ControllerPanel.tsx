...
function ControlButtonGroup({ isAnalog, deviceKey, buttonKey }) {
	const touchRef = React.useRef<HTMLButtonElement>();
	const pressRef = React.useRef<HTMLButtonElement>();
	const holdRef = React.useRef<HTMLButtonElement>();
	const rangeRef = React.useRef<HTMLInputElement>();
	...
	function onRangeInput() {
		const inputValue = parseInt(rangeRef.current.value) / 100;
		applyControllerButtonChanged(
			deviceKey,
			BUTTON_POLYFILL_INDEX_MAPPING[buttonKey],
			inputValue != 0,
	...
	return (
		<div className="control-button-group">
			...
			>
				<img src="./res/icons/xr-emulator/press.png" />
			</button>
			<button
				className="btn special-button"
				ref={pressRef}
			...
			{isAnalog ? (
				...
				>
					<img src="./res/icons/xr-emulator/lock.png" />
				</button>
			)}
		</div>
	);
...
export function ControllerPanel({ deviceKey }) {
	const strings = CONTROLLER_STRINGS[deviceKey];
	const joystickContainerRef = React.useRef<HTMLDivElement>();
	const joystickResetRef = React.useRef<HTMLButtonElement>();
	const joystickStickyRef = React.useRef<HTMLButtonElement>();
					...
					<div className="card-body">
						...
						<div className="row">
							<div className="col-12 d-flex justify-content-end">
								<div className="control-button-group">
									...
									>
										<img
											src="./res/icons/xr-emulator/sticky.png"
									...
									>
										<img
											src="./res/icons/xr-emulator/reset.png"
											className="action-icon"
										/>
									</button>
								</div>
						...
						{['button2', 'button1'].map((controlName) => (
							<div key={controlName} className="row">
								<div className="col-4 d-flex align-items-center">
									<img
										src={`./res/icons/xr-emulator/${controlName}-${strings.handedness}.png`}
										className="control-icon"
									/>

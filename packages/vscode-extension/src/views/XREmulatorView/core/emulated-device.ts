...
import { EmulatorSettings, emulatorStates } from './emulator-states';
...
export default class EmulatedDevice extends EventEmitter {
  ...
  _transformControls: { [key: string]: TransformControls };
  _controllerMeshes: THREE.Mesh[];
  _handMeshes: THREE.Mesh[];
  _labelContainer: HTMLDivElement;
  _raycaster: THREE.Raycaster;
  _mouseVec2: THREE.Vector2;
  _mouseDownTime: number;
  _selectedDeviceKey: string;
  _roomObject: THREE.LineSegments;
  _userObjects: { [key: string]: THREE.Object3D };
  _lastWidth: number;
  _lastHeight: number;
  ...
  constructor() {
    ...
    Object.values(DEVICE).forEach((deviceKey) => {
      ...
      loader.load(`./res/3d/${OBJECT_NAME[deviceKey]}.glb`, (gltf) => {
      ...
      if (HAND_STRINGS[deviceKey]) {
        loader.load(`./res/3d/${HAND_STRINGS[deviceKey].name}.glb`, (gltf) => {
          const mesh = gltf.scene;
          mesh.scale.setScalar(2);
          mesh.rotateY(Math.PI);
          mesh.traverse((child) => {

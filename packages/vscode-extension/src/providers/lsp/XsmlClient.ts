...
import { FileSystemProvider, serveFileSystemRequests } from './RequestInterfaces';
...
namespace CustomDataChangedNotification {
  export const type: NotificationType<string[]> = new NotificationType('xsml/customDataChanged');
}
namespace CustomDataContent {
  export const type: RequestType<string, string, any> = new RequestType('xsml/customDataContent');
}
...
namespace AutoInsertRequest {
  export const type: RequestType<AutoInsertParams, string, any> = new RequestType('xsml/autoInsert');
}
...
namespace SemanticTokenRequest {
  export const type: RequestType<SemanticTokenParams, number[] | null, any> = new RequestType('xsml/semanticTokens');
}
namespace SemanticTokenLegendRequest {
  export const type: RequestType0<{ types: string[]; modifiers: string[] } | null, any> = new RequestType0('xsml/semanticTokenLegend');
}

namespace SettingIds {
  export const linkedEditing = 'editor.linkedEditing';
  export const formatEnable = 'xsml.format.enable';
}

export type LanguageClientConstructor = (name: string, description: string, clientOptions: LanguageClientOptions) => BaseLanguageClient;
export const languageServerDescription = l10n.t('JSAR XSML Language Server');
...
export async function startClient(context: ExtensionContext, newLanguageClient: LanguageClientConstructor, runtime: Runtime): Promise<AsyncDisposable> {
  ...
  const promptForLinkedEditingKey = 'xsml.promptForLinkedEditing';
  if (extensions.getExtension('formulahendry.auto-rename-tag') !== undefined && (context.globalState.get(promptForLinkedEditingKey) !== false)) {
    const config = workspace.getConfiguration('editor', { languageId: 'xsml' });
  ...
  languageParticipants.onDidChange(() => {
    ...
    restartTrigger = runtime.timer.setTimeout(async () => {
      if (client) {
        outputChannel.appendLine('Extensions have changed, restarting XSML server...');
...
async function startClientWithParticipants(languageParticipants: LanguageParticipants, newLanguageClient: LanguageClientConstructor, outputChannel: OutputChannel, runtime: Runtime): Promise<AsyncDisposable> {
  ...
  const clientOptions: LanguageClientOptions = {
    documentSelector,
    synchronize: {
      configurationSection: ['xsml', 'css', 'javascript', 'js/ts'], // the settings to synchronize
    },
    initializationOptions: {
      embeddedLanguages,
      handledSchemas: ['file'],
      provideFormatter: false, // tell the server to not provide formatting capability and ignore the `xsml.format.enable` setting.
  ...
  const client = newLanguageClient('xsml', languageServerDescription, clientOptions);
  ...
  // manually register / deregister format provider based on the `xsml.format.enable` setting avoiding issues with late registration. See #71652.
  ...
  const xsmlSnippetCompletionRegExpr = /^(\s*)(<(h(t(m(l)?)?)?)?)?$/;
  toDispose.push(languages.registerCompletionItemProvider(documentSelector, {
    provideCompletionItems(doc, pos) {
      ...
      const match2 = lineUntilPos.match(xsmlSnippetCompletionRegExpr);
      if (match2 && doc.getText(new Range(new Position(0, 0), pos)).match(xsmlSnippetCompletionRegExpr)) {
        const range = new Range(new Position(pos.line, match2[1].length), pos);
        const snippetProposal = new CompletionItem('XSML sample', CompletionItemKind.Snippet);
        ...
        snippetProposal.documentation = l10n.t('Simple XSML starting point');
